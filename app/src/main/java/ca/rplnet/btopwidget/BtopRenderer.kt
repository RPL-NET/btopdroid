...
object BtopRenderer {
    ...
    private fun bar(pct: Int, filledChar: String, emptyChar: String): String {
        val clamped = pct.coerceIn(0, 100)
        val filled = (clamped * BAR_WIDTH) / 100
        return "[" + filledChar.repeat(filled) + emptyChar.repeat(BAR_WIDTH - filled) + "]"
    }

    fun render(s: Stats, filledChar: String, emptyChar: String): String {
        val sb = StringBuilder()
        sb.append("${s.clock}  ${s.date}\n")
        sb.append("uptime ${s.uptimeStr}\n")
        sb.append("─".repeat(BAR_WIDTH + 8)).append("\n")

        sb.append("ram  ${bar(s.ramUsedPct, filledChar, emptyChar)} ${s.ramUsedPct}%\n")
        sb.append("     ${s.ramUsedMb}M / ${s.ramTotalMb}M\n")

        sb.append("dsk  ${bar(s.storageUsedPct, filledChar, emptyChar)} ${s.storageUsedPct}%\n")
        sb.append("     %.1fG / %.1fG\n".format(s.storageUsedGb, s.storageTotalGb))

        val cpuLine = if (s.cpuPct != null) {
            "cpu  ${bar(s.cpuPct, filledChar, emptyChar)} ${s.cpuPct}%\n"
        } else {
        ...
        sb.append("bat  ${bar(s.batteryPct, filledChar, emptyChar)} ${s.batteryPct}%$battIcon\n")
